<template>
  ...
  >
    ...
    <!-- BOTTOM: attachments, toolbar, actions -->
      ...
      <div v-if="editable" class="flex flex-col gap-2">
        ...
        <div class="flex flex-wrap gap-2 sm:px-10 px-4">
          <AttachmentItem
            ...
            :key="a.file_url || a.name"
            :label="a.file_name || a.name"
          >
            <template #suffix>
              <FeatherIcon
                name="x"
                class="h-3.5 cursor-pointer text-ink-gray-6 hover:text-ink-gray-9"
...
import { TextEditorBubbleMenu, TextEditor, FileUploader, call, Button } from 'frappe-ui'
...
const editor = computed(() => textEditor.value?.editor || null)
...
async function applyEmailTemplate(template) {
  ...
  if (template.response && data?.message) {
...
function appendEmoji() {
  if (!emoji.value || !editor.value?.commands) return
...
/* expose for parent (CommunicationArea.vue etc.) */
defineExpose({
  editor,
  subject,
  cc,
